...
def test_max_available_gain_does_not_exist_below_k_of_one():
    ...
    with pytest.raises(ValueError, match="potentially unstable and has no maximum"):
...
def test_a_unilateral_device_has_no_feedback_and_so_no_k():
    ...
    assert st.max_stable_gain_db(unilateral) == math.inf
    assert st.max_available_gain_db(unilateral) == math.inf
